...
export default function Leaderboard() {
  ...
  useEffect(() => { fetch(`${apiBaseUrl}/api/leaderboard/`).then((response) => { if (!response.ok) throw new Error('Unable to load leaderboard'); return response.json() }).then((payload) => { setEntries(getRecords(payload).sort((a, b) => (a.rank || 999) - (b.rank || 999))); setState({ loading: false, error: '' }) }).catch((error) => setState({ loading: false, error: error.message })) }, [])
