...
export default function Leaderboard() {
  ...
  useEffect(() => { fetchRecords(endpoint).then((data) => { setEntries(data.sort((a, b) => (a.rank || 999) - (b.rank || 999))); setState({ loading: false, error: '' }) }).catch((error) => setState({ loading: false, error: error.message })) }, [])
